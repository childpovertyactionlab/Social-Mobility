...
tract_housing <- tract.select %>%
  ...
  mutate(HCV_rohh = HCV_PUBLIC/rohhE_18)
...
sdcut <- function(x) {
  ...
  sd2p <- mean(x, na.rm = TRUE)+(sd(x, na.rm = TRUE)*0.9)
  sd2n <- mean(x, na.rm = TRUE)-(sd(x, na.rm = TRUE)*0.9)
  ifelse(x > sd2p, 4,
