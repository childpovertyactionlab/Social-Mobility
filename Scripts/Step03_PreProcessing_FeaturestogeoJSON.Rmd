...
names(childcareAll.sh)
childcareAll.join <- childcareAll.sh %>%
  filter(USER_SubsY == "N") %>%
  mutate(Display = "Unsubsidized Childcare Center",
...
featureset <- rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(
                    snap.zip, banks.zip), comcen.zip), comhealth.zip), creditunions.zip), supermarket.zip), pharmacies.zip), libraries.zip), schools.zip), wic.zip), childcare.zip)#, childcareAll.zip)
