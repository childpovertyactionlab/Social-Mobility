...
snap.zip <- mutate(snap.zip, FID = 1:n())

banks.zip <- banks.join[zip.sh, ]
banks.zip <- mutate(banks.zip, FID = 1:n())

comcen.zip <- comcen.join[zip.sh, ]
comcen.zip <- mutate(comcen.zip, FID = 1:n())

comhealth.zip <- comhealth.join[zip.sh, ]
comhealth.zip <- mutate(comhealth.zip, FID = 1:n())

creditunions.zip <- creditunions.join[zip.sh, ]
creditunions.zip <- mutate(creditunions.zip, FID = 1:n())

supermarket.zip <- supermarket.join[zip.sh, ]
supermarket.zip <- mutate(supermarket.zip, FID = 1:n())

pharmacies.zip <- pharmacies.join[zip.sh, ]
pharmacies.zip <- mutate(pharmacies.zip, FID = 1:n())

libraries.zip <- libraries.join[zip.sh, ]
libraries.zip <- mutate(libraries.zip, FID = 1:n())
...
wic.zip <- mutate(snap.zip, FID = 1:n())

childcare.zip <- childcare.join[zip.sh, ]
childcare.zip <- mutate(childcare.zip, FID = 1:n())
```

```{r}
schools.zip <- rbind(rbind(rbind(rbind(rbind(ihs, ims), ies), chs), cms), ces)
