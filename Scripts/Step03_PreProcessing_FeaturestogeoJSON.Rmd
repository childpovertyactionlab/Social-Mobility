...
vita.zip <- vita.join[zip.sh, ]
rm(vita.join)

fqhc.zip <- fqhc.join[zip.sh, ]
rm(fqhc.join)
```
...
featureset <- rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(rbind(
                    banks.zip, comcen.zip), comhealth.zip), creditunions.zip), supermarket.zip), pharmacies.zip), libraries.zip), schools.zip), schoolsprivate.zip), wic.zip), childcare.zip), childcareAll.zip), vita.zip), fqhc.zip)
